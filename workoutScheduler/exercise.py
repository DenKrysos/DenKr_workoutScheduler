...
class exercise(CommonMuscleExercise):
    ...
    def _set_exercises__strip_enum(cls):
        for i in range(len(cfghandle.cfgSetup_handle[cfghandle.keySetupExe])):
            # equip=exercise_setup[i][3]
            # for j in range(len(equip)):
            #     equip[j]=equip[j].value[0]
            muscle=cfghandle.cfgSetup_handle[cfghandle.keySetupExe][i][4]
    ...
    def set_exercises(cls,trgtArr,excludedArr):
        ...
        for index, (ident, enabled, precedence, equip, muscle) in enumerate(cfghandle.cfgSetup_handle[cfghandle.keySetupExe]):
    ...
    def derive_urgency(self,credit_center,wo_perWeek_total):
        ...
        #     if muscle[0]==muscleID.chest.value[0]:
